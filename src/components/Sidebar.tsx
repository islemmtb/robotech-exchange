...
type Item = { key: NavKey; href: string; ready: boolean; icon: React.ReactNode };
  ...
  return (
    <nav className="flex flex-1 flex-col gap-0.5">
      {items.map((it) => {
        if (!it.ready) {
          return (
            <span key={it.key} className={base + " cursor-default text-muted/60"}>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          <div
            ...
            onClick={onClose}
          />
          <aside className="absolute left-0 top-0 flex h-full w-64 flex-col border-r border-[var(--border)] bg-surface px-3 py-4 shadow-2xl">
